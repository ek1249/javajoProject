...
<hr>
<table width="70%" cellpadding="0" cellspacing="0" align="center" border="1">
	<tr>
		<td>게시글 번호</td>
		<td>게시글 제목</td>
		<td>게시글 작성자</td>
	...
	<m:forEach var="p" items="${list }">
		<tr>
			<td>${p.postboard_number }</td>
			<td>
			<m:if test="${p.postboard_p_level>0 }">
				<m:forEach var="i" begin="1" end="${p.postboard_p_level }">
					&nbsp;&nbsp;
				</m:forEach>
			</m:if>
			<a href="postboarddetail.com?no=${p.postboard_number }">${p.postboard_title }</a></td>
			<td>${p.customer_id }</td>
...
<table align="right">
	<tr>
		<td><a href="javajo/postboardinsert.jsp?id=${id }"><input type="button" value="문의하기"></a></td>
	</tr>
</table>
</body>
</html>